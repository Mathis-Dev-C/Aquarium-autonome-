...
{
	if (dimState[this->current_num] == ON1)
...
{
	bool ret;
	if (dimState[this->current_num] == ON1) ret = true;
	else ret = false;
	return ret;
}
...
{
	if (dimState[this->current_num] == ON1) dimState[this->current_num] = OFF1;
	else 
		dimState[this->current_num] = ON1;
}
